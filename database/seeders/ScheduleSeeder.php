    ...
    public function run(): void
    {
        ...
        for($i = 1; $i <= 10; $i++) {
			...
			for($appt_index = 0; $appt_index < rand(1, 3); $appt_index++) {
				...
				$start = Carbon::parse($date.' 10:00')->addMinutes($appt_index * 15)->format('Y-m-d H:i:s');
				$end = Carbon::parse($start)->addMinutes($appt_index + 15)->format('Y-m-d H:i:s');
				
				Appointment::create([
					'schedule_id' => $schedule->id,
					'patient_id' => User::where('role', UserRole::Patient)->get()->random()->id,
